...
@section('content')
<a href="{{ route('superadmin.onboarding.index') }}" class="back-link">
            ...
            @foreach($company->kycDocuments as $doc)
                <div class="doc-item">
                    ...
                    <div class="doc-item-body">
                        <strong>{{ $doc->type === 'national_id' ? 'Government ID' : 'Company Registration Documents' }}</strong>
                        @if($doc->subtypeLabel())
                            <span class="cell-sub">{{ $doc->subtypeLabel() }}</span>
                        @endif
                        <span>{{ $doc->original_name }} &middot; by {{ $doc->uploader->name }}</span>
                    </div>
                    <a href="{{ route('superadmin.kyc.download', $doc) }}" class="btn btn-secondary btn-sm">Download</a>
                </div>
            @endforeach
        </div>
    @endif
...
<div class="card review-actions-card">
    ...
    <div class="review-actions">
        <form method="POST" action="{{ route('superadmin.onboarding.approve', $company) }}" class="approve-form">
            @csrf
        ...
        <form method="POST" action="{{ route('superadmin.onboarding.reject', $company) }}" class="reject-form">
            @csrf
