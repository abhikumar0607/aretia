...
@section('content')
        ...
        <div class="listing-panel">
            @include('partials.listing-toolbar', [
                'action' => route('superadmin.clients.index'),
                        ...
                        <tr class="data-table-row">
                            ...
                            <td class="cell-action">
                                @if($onboardingReview && $company)
                                    <a href="{{ route('superadmin.onboarding.show', $company) }}" class="btn btn-secondary btn-sm">Review</a>
                                @else
                                    @include('partials.user-account-actions', [
                                        'user' => $clientUser,
                                        'canManage' => $canManage,
                                        'deactivateRoute' => route('superadmin.users.deactivate', $clientUser),
                                        'activateRoute' => route('superadmin.users.activate', $clientUser),
                                        'deleteRoute' => route('superadmin.users.destroy', $clientUser),
                            ...
                            <td colspan="6">
                                <div class="empty-state">
                                    @if($hasFilters)
                                        ...
                                        <a href="{{ route('superadmin.clients.index') }}" class="btn btn-secondary btn-sm">Clear filters</a>
                                    @else
