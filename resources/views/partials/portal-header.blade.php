@auth
@php
    $headerUser = auth()->user();
    $showChatInbox = in_array($headerUser->role->value, ['client', 'analyst'], true);
@endphp
<div class="portal-topbar">
    <div class="portal-topbar-inner">
        <div class="portal-topbar-actions">
            @if($showChatInbox)
                @include('partials.chat-bell-inner')
            @endif
            @include('partials.notification-bell-inner')

            <div class="portal-user-menu" id="portal-user-menu">
                <button type="button" class="portal-icon-btn portal-settings-btn" id="portal-settings-toggle" aria-label="Account menu" aria-expanded="false" aria-haspopup="true">
                    <svg width="22" height="22" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/>
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                    </svg>
                </button>
                <div class="portal-user-dropdown" id="portal-user-dropdown" hidden>
                    <div class="portal-user-dropdown-head">
                        @if($headerUser->avatarUrl())
                            <img src="{{ $headerUser->avatarUrl() }}" alt="" class="portal-user-dropdown-avatar">
                        @else
                            <span class="portal-user-dropdown-initials">{{ $headerUser->initials() }}</span>
                        @endif
                        <div>
                            <strong>{{ $headerUser->name }}</strong>
                            <span>{{ $headerUser->email }}</span>
                        </div>
                    </div>
                    <a href="{{ route('profile.edit') }}" class="portal-user-dropdown-item">
                        <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                        My Profile
                    </a>
                    <form method="POST" action="{{ route('logout') }}" class="portal-user-dropdown-logout">
                        @csrf
                        <button type="submit" class="portal-user-dropdown-item portal-user-dropdown-logout-btn">
                            <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/></svg>
                            Logout
                        </button>
                    </form>
                </div>
            </div>
        </div>
    </div>
</div>
@endauth
